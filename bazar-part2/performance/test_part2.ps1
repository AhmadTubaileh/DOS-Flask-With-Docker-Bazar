# PowerShell Performance Test for Part 2 (No Python Required)
$FRONTEND_URL = "http://localhost:5002"
$NUM_REQUESTS = 500  # Increased to show cache effectiveness better

$separator = "=" * 70
Write-Host $separator -ForegroundColor Cyan
Write-Host "BAZAR Part 2 Performance Test (With Caching and Replication)" -ForegroundColor Cyan
Write-Host $separator -ForegroundColor Cyan
...
# Test Info Endpoint
Write-Host "`nTesting Info Query (with cache)..." -ForegroundColor Yellow
Write-Host "Note: First request = cache miss, subsequent = cache hits" -ForegroundColor Gray
$infoTimes = @()
$infoSuccess = 0
...
Write-Host "`nTesting Search Query (with cache)..." -ForegroundColor Yellow
$searchTimes = @()
$searchSuccess = 0
...
Write-Host "`nTesting Purchase Operation (with cache invalidation)..." -ForegroundColor Yellow
$purchaseTimes = @()
$purchaseSuccess = 0
$purchaseCount = 20
